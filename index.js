...
app.post("/api/process", async (req, res) => {
  try {
    ...
    // Extract the 'identity' section from the API response
    const identity = realEstateResponse.data.output?.identity;

    // Simplify the identity object
    const simplifiedIdentity = {
      name: identity.names ? identity.names[0] : null,
      address: identity.address || null,
      addressHistory: identity.addressHistory || null,
      phone: identity.phones ? identity.phones[0] : null,
      email: identity.emails ? identity.emails[0] : null,
      OldEmail: requestBody
    };

    // Log the simplified identity section as a JSON object
    console.log('Simplified Identity:', JSON.stringify(simplifiedIdentity, null, 2));
    ...
    //console.log("RealEstate API response:", realEstateResponse.data.body);
